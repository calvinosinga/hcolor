...
try:
    ...
    try:
        ...
        w.close()
        wzip = zf.ZipFile(SAVE+'nelson_'+str(SNAPSHOT)+'.'+str(CHUNK)+'.hdf5.zip', 'w')
        wzip.write(SAVE+'nelson_'+str(SNAPSHOT)+'.'+str(CHUNK)+'.hdf5', compress_type=zf.ZIP_DEFLATED)
        wzip.close()
        os.remove(SAVE+'nelson_'+str(SNAPSHOT)+'.'+str(CHUNK)+'.hdf5')