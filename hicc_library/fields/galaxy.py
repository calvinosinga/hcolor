...
class galaxy(Field):
    ...
    def getResolutionDefinitions(simname):
        ...
        # wigglez
        galaxy_min_resolution['wiggleZ'] = {'r':20, 'r':22}
    ...
    def _loadGalaxyData(self, fields):
        data = super()._loadGalaxyData(fields)
        pos = data['SubhaloPos'][:]
        vel = data['SubhaloVel'][:]
    ...
    def computeGrids(self):
        ...
        pos, vel, mass, photo = self._loadGalaxyData(fields)
        ...
        for col in self.col_defs:
            ...
            for g in self.gridnames:     
                mask = mask_dict[g]           
                self._computeGal(pos[mask], mass[mask], g+'_'+col)
                self.saveData(col)
        ...
        for col in self.col_defs:
            ...
            for g in self.gridnames:
                mask = mask_dict[g]
                self._computeGal(pos[mask], mass[mask], g+'_'+col) # rs added in grid's save method
                self.saveData(col)
    ...
    def _computeGal(self, pos, mass, gridname):
...
class galaxy_dust(galaxy):
    def __init__(self, gd, simname, snapshot, axis, resolution, outfile):
        ...
        self.dustfilepath = gd['dust']
        return
    
    def _loadGalaxyData(self, fields):
        pos, vel, mass, photo = super()._loadGalaxyData(fields)
        dustfile = hp.File(self.dustfilepath, 'r')
