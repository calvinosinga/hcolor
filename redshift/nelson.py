...
print('The MAS given is:%s'%MAS)
for i in range(FILENO):
    try:
        f = hp.File(HOME+'fof_subhalo_tab_0'+str(SNAPSHOT)+'.'+str(i)+'.hdf5','r')
    except IOError:
        print('failed to open file for '+str(i)+'\n')
    else:
        try:
            ...
            print("chunk "+str(i)+ '\'s subhalo data was empty\n')
        else:
            #get needed constants
            cons = f['Header'].attrstype
            ...
            print("starting procedure for chunk %d"%i)

            # move to redshift space
            rsl.pos_redshift_space(pos,vel,BOXSIZE,HUBBLE, redshift, AXIS)

            gr = photo[:,4]-photo[:,5]
            total_mass = np.sum(mass,axis=1) #still float32
            stmass = mass[:,4]
            ...
            # nondetpos = pos[nondetection_idx]
            # nondetmass = total_mass[nondetection_idx]
